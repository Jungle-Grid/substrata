...
export class NoRemoteProviderAvailable extends Error {
  constructor() {
    super(
      'No remote execution provider is currently available. Configure Fireworks, Jungle Grid, or AMD Notebook manual mode.',
    );
  }
}
...
function jungleGridEnabled() {
  const canonical = process.env.JUNGLE_GRID_ENABLED?.trim();
  return canonical
    ? enabled('JUNGLE_GRID_ENABLED')
    : enabled('JUNGLEGRID_ENABLED');
}

function remotePriority() {
  return (
    process.env.REMOTE_PROVIDER_PRIORITY ??
    'junglegrid,fireworks,amd_notebook_manual'
  )
    .split(',')
...
export function selectExecutionProvider(executionMode: ExecutionMode): {
  ...
  if (executionMode === 'local') {
    if (
      !enabled('LOCAL_GEMMA_ENABLED') &&
      process.env.LOCAL_GEMMA_ENABLED !== undefined
    ) {
      throw new Error(
    ...
    return {
      ...
      providerSelectionReason:
        'Local execution selected; Gemma Local is required.',
  ...
  for (const provider of priority) {
    if (
      provider === 'junglegrid' &&
      jungleGridEnabled() &&
      Boolean(process.env.JUNGLE_GRID_API_KEY)
    ) {
      return {
        executionMode,
        selectedProvider: provider,
        providerSelectionReason:
          'Selected the first enabled remote provider in priority order.',
        providerPriority: priority,
      };
    }
    if (
      provider === 'fireworks' &&
      (process.env.FIREWORKS_ENABLED === undefined ||
    ...
    if (
      provider === 'amd_notebook_manual' &&
      enabled('AMD_NOTEBOOK_MANUAL_ENABLED')
    ) {
      return {
        executionMode,
        selectedProvider: provider,
        providerSelectionReason:
          'Selected explicitly enabled AMD Notebook manual provider.',
        providerPriority: priority,
      };
    }
  }
